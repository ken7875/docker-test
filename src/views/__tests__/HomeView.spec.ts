...
describe('home test', () => {
  ...
  it('test promise has been called', async () => {
    const spy = vi.spyOn(axios, 'get')
    const wrapper = createWrapper()
    await flushPromises()
    expect(spy).toHaveBeenCalledTimes(1)
    expect(spy).toHaveBeenCalledWith('http://localhost:8000/api/products')
  })

  it('test promise has been called', async () => {
    const spy = vi.spyOn(axios, 'get')
    const wrapper = createWrapper()
    spy.mockImplementation(() => Promise.resolve({ data: { data: { data: productsData } } }))
    await flushPromises()
